	...
	<div class="space-y-4 text-center size-full">
		<div
			...
			<div class="flex flex-col items-center justify-center h-full w-1/4 rounded-tl rounded-bl pl-10 pr-10">
				<el-button @click="modifyVSProportion" v-if="isFileLoading1">重新加载</el-button>
			</div>
		</div>
		...
		<div class="flex items-center p-4 rounded h-24">
			<el-button @click="lastProcess">生成处理后的文件</el-button>
			<p class="pl-6 text-green-600">最后生成的文件会直接下载</p>
		</div>
	</div>
</template>
...
const lastProcess = () => {
	if (Object.keys(selectedData.value).length === 0) {
		...
		for (let item of timeRangeList.value) {
			startTimeList.push(item[0]);
			endTimeList.push(item[1]);
		}
